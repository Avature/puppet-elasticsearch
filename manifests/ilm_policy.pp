...
#  life cycle ploicies.
#
#  ILM policy content should be defined through the `content` parameter
#  (when passing a hash or json string)
#
# @param ensure
...
#
# @author Gustavo Yoshizaki <gustavoyy@gmail.com>
#
define elasticsearch::ilm_policy (
  ...
  if $ensure == 'present' and $_content == undef {
    fail('"content" required.')
  }

  es_instance_conn_validator { "${name}-ilm_policy":
    server  => $api_host,
    port    => $api_port,
    timeout => $api_timeout,
